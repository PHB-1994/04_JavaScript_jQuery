...
$(function () {
  ...
  $("#nextBtn").click(() => {
    // 다음 페이지로 가기
    // 만약에 가장 마지막 페이지 이면 마지막 다음 페이지 입니다.
    현재페이지++;
    pokeInfo(현재페이지);
  });
});
...
function range(start, end) {
  /*
    ...
    .keys() : 키들의 값으로 숫자를 넣겠다. 배열은 0 부터 시작 [0, 1, 2, ... , 7, 8, 9]    

    map((i) => i + start) : 시작값 더하기
            포켓몬 사이트는 0 이 존재하지 않고 1부터 시작하기 때문에
            배열은 0 이지만 0 번째에 https://pokeapi.co/api/v2/pokemon/1 의 값이 들어가게 되는 것

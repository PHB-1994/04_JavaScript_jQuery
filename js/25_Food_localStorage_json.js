...
function addFoodData(e) {
  ...
  const newFood = {
    ...
    price: price,
    category: category,
    createAt: new Date().toLocaleString("ko-KR"), // 저장시간 현재 시간으로 한국 기준 설정
  };

  // js 에서 foolList 목록에 newFood 데이터 맨 마지막에 추가
  // 추가된 foodList 를 localStorage 에 글자열로 변환하여 지정
  // foodName pirce createAt 입력창 비우기
...
function searchFoodData(e) {
  ...
  $("#searchResult").html(html).show(); // display : none 를 볼 수 있도록 설정하는 기능
}
...
function showAllFoodData(e) {
  ...
  showAllFoodData(e) : 행동이 들어왔을 때
    ...
    undefined (행동이 전달되지 않는 상태 / 단순 페이지 오픈상태)

    if (e) e.preventDefault();
      클릭과 같은 동작이 들어왔을 때 태그 속성을 멈추기 위하여 설정
  
      if 로 e.preventDefault() 를 감싸주지 않는다면
      지금과 같이

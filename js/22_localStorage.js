...
// 키 값은 문자열로 무조건 되어있음
// "" 가 없다하여 문자열이 아닌 것이 아니라 생략 상태!!!

// 인터넷 창에서 크롬일 경우 chrome: // version 입력
// C:\Users\tj\AppData\Local\Google\Chrome\User Data\Default
// 프로필 경로를 따라서 폴더 내부를 들어가면 local Storage 폴더가 존재할 것
// 폴더 내부에 db 나 dbl 로 저장된 내부에 컴퓨터가 읽을 수 있는 글씨로 존재
// localStorage 내부에서는 문자열만 저장이 가능 = 인터넷 주소 즐겨찾기 나 간단한 읽는 정도의 데이터를 저장하므로
// 소비자가 인터넷을 사용함에 있어 불편을 느끼지 않을 최소한의 저장 자료형을 사용!
// 악성 코드 방지 = 문자열만 가능하고 배열 또한 문자열로 저장 ""

function saveDataFn(e) {
  // button type = submint 이거나 a 태그로 클릭할 경우 제출 방지 사용
  // a 태그 내부 속성 기본값 href 로 들어가는 것을 방지 (예: 마이페이지, 임직원페이지 접근 가능한 유저인가?)
  e.preventDefault(); // 제출 방지 기능 type="button" 일 경우 사용 안해도 됨
...
// 키의 이름을 가져올 때는 index 번호를 활용해서 0 번째 존재하는 key 명칭을 가져온다.
// 가지오 온 키의 명칭을 값을 가져올 수 있다.
// getkey = index 번호
// getvalue = key 의 명칭
...
function clearAllDataFn(e) {
  ...
  if (confirm("정말로 모든 데이터를 삭제하시겠습니다?")) {
    localStorage.clear();
  }
  showAllDataFn();
}

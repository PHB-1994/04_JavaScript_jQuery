...
function getUsers() {
  ...
  $.get("https://jsonplaceholder.typicode.com/users").done(function (data) {
     ...
     * getUserDetail()   -> data 에서 가져온 유저 목록들을 순회하여
     *                      user 1명씩 보유하고 있는 id 를 이용해서
     *                      주소값으로 사용
     *                      map 으로 유저 정보를 하나씩 보여줄 때 유저 데이터에는
     *                      id, name, username, email, address 등의 데이터를 보유
     *         관리자가 보길 원하는 회원 유저를 클릭했을 때
